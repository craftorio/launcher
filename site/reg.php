...
if (preg_match("/(admin|helper|member|owner)/i", $login)) die ("errorLoginSymbol");
elseif (!preg_match("/^[a-zA-Z0-9_-]+$/", $password)) die ("passErrorSymbol");
elseif (!preg_match("/^[0-9a-z_\-.]+@[0-9a-z_\-^\.]+\.[a-z]{2,}$/i", $mail)) die ("errorMail");
...
try {
    ...
    $sql = "INSERT INTO $db_table ($db_columnUser,$db_columnPass,$db_columnMail, activated_at, created_at, updated_at, $db_columnIp, is_activated) VALUES(:login,:checkPass,:mail,NOW(),NOW(),NOW(),'$ip', '1')";
