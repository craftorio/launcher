...
public class ThreadUtils {
                ...
                try {
                    ...
                    if (error) {
                        ...
                        if (personal) {
                            ...
                            String personal = BaseUtils.execute(BaseUtils.buildUrl("launcher.php"), new Object[]{"action", encrypt("getpersonal:0:" + Frame.login.getText() + ":" + token, Settings.key2),});
    ...
    public static void upload(final File file, final int type) {
        new Thread() {
            public void run() {
                ...
                String answer = BaseUtils.execute(buildUrl("launcher.php"), new Object[]{"action", encrypt(get + ":0:" + Frame.login.getText() + ":" + token, Settings.key2), "ufile", bytes64});
    ...
    public static void vaucher(final String vaucher) {
        new Thread() {
            public void run() {
                String answer = BaseUtils.execute(buildUrl("launcher.php"), new Object[]{"action", encrypt("activatekey:0:" + Frame.login.getText() + ":" + token, Settings.key2), "key", vaucher,});
                boolean error = false;
                if (answer == null) {
                    Frame.main.panel.tmpString = Message.Null;
                    error = true;
    ...
    public static void exchange(final String text) {
        new Thread() {
            public void run() {
                String answer = BaseUtils.execute(buildUrl("launcher.php"), new Object[]{"action", encrypt("exchange:0:" + Frame.login.getText() + ":" + token, Settings.key2), "buy", text,});
                boolean error = false;
                if (answer == null) {
                    Frame.main.panel.tmpString = Message.Null;
                    error = true;
    ...
    public static void buyVip(final int i) {
        new Thread() {
            public void run() {
                ...
                String answer = BaseUtils.execute(buildUrl("launcher.php"), new Object[]{"action", encrypt(z + ":0:" + Frame.login.getText() + ":" + token, Settings.key2),});
                boolean error = false;
                if (answer == null) {
                    Frame.main.panel.tmpString = Message.Null;
                    error = true;
    ...
    public static void unban() {
        new Thread() {
            public void run() {
                String answer = BaseUtils.execute(buildUrl("launcher.php"), new Object[]{"action", encrypt("buyunban:0:" + Frame.login.getText() + ":" + token, Settings.key2),});
                boolean error = false;
                if (answer == null) {
                    Frame.main.panel.tmpString = Message.Null;
                    error = true;
