    ...
    public void paintComponent(Graphics gmain) {
        ...
        if (pressed) {
            ...
            if (entered && y / pressedTX.getHeight() < elements.length) {
                g.drawImage(genButton(w, selectedTX.getHeight(), selectedTX), 0, y / pressedTX.getHeight() * pressedTX.getHeight(), w, pressedTX.getHeight(), null);
            }

            for (int i = 0; i < elements.length; i++) {
                g.drawString(elements[i], padding, selectedTX.getHeight() * (i + 1) - (g.getFontMetrics().getHeight() / 2));
            }

            g.drawString(elements[selected], padding, selectedTX.getHeight() * (elements.length + 1) - (g.getFontMetrics().getHeight() / 2) -2);
        } else if (entered) {
            ...
            g.drawString(elements[selected], padding, rolloverTX.getHeight() - (g.getFontMetrics().getHeight() / 2) -2);
        } else {
            int righth = pressedTX.getHeight();
            if (getY() != initialy || getHeight() != righth) {
                setLocation(getX(), initialy);
